...
let loaded = 0;
[dinoImg,cactusImg,birdImg,bgImg].forEach(i=>{
  i.onload = ()=>{ if(++loaded===4) state=MENU; }
});
...
/* ========= DINO ========= */
const dino = {
  x:320-37,y:groundY,w:74,h:72,
  vx:0,vy:0,speed:4,
  g:0.7,jump:-12,
  grounded:true,dir:1,alive:true,
...
let enemies=[];
let gameTime=0;
let spawnCooldown=90; // 3 segundos
...
function loop(time){
  ...
  if(state===GAME){
    ...
    if(gameTime%spawnCooldown===0) spawnEnemy();
  ...
  if(state===PAUSE) text("PAUSA",180,24);
  if(state===OVER) text("GAME OVER",180,24);
